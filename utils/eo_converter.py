...
def main():
    ...
    with h5py.File(DATASET_DIR + "slovenia2017.hdf5", "w") as f:
        ...
        for i, patch_name in enumerate(os.listdir(DATASET_DIR)):
            ...
            patch_indices = extract_name(patch_name)

            # Write raw bands data (images)
            data = eo_patch.data['BANDS']
            subset = write_to_hdf5(h5set, 'data_bands', patch_indices, data=data)

            # Write mask
            data = eo_patch.mask['CLM']
            subset = write_to_hdf5(h5set, 'mask/clm', patch_indices, data=data)

            data = eo_patch.mask['IS_DATA']
            subset = write_to_hdf5(h5set, 'mask/is_data', patch_indices, data=data)

            data = eo_patch.mask['VALID_DATA']
            subset = write_to_hdf5(h5set, 'mask/valid_data', patch_indices, data=data)

            # Write timeless mask
            data = eo_patch.mask_timeless['LULC']
            subset = write_to_hdf5(h5set, 'mask_timeless/lulc', patch_indices, data=data)

            data = eo_patch.mask_timeless['VALID_COUNT']
            subset = write_to_hdf5(h5set, 'mask_timeless/valid_count', patch_indices, data=data)
...
def write_to_hdf5(dset,
    ...
    # Create dataset if it doesn't exists
    if subset_name not in dset.keys():
        shape = (PATCHES_W, PATCHES_H) + data.shape
        chunk_shape = (1, 1) + data.shape
        dtype = data.dtype
        subset = dset.create_dataset(subset_name,
                                 dtype=dtype,
                                 shape=shape,
                                 chunks=chunk_shape,  # 10 images per chunk
                                 )
    else:
        subset = dset[subset_name]
